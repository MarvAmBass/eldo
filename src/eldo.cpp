...
void printInfo() {
  ...
  std::cout << current_exec_name << " (gpio) (elro_code):(status [1|0])"
            << std::endl;
  std::cout << current_exec_name << " 17 10101A:1" << std::endl;
  std::cout << std::endl;
  std::cout << "multicast client (listening on udp port " << port
            << "):" << std::endl;
...
int send_multicast(int reps, char *cmd_cstring[]) {
  ...
  for (int i = 0; i < reps; i++) {
    if (sendto(socket_descriptor, echo_string, echo_len, 0,
               (struct sockaddr *)&address, sizeof(address)) < 0) {
      std::cout << "sendto failure" << std::endl;
      return 1;
    }
    std::cout << "send command #" << (i + 1) << " " << echo_string
              << " to clients" << std::endl;
    sleep(1);
  }
